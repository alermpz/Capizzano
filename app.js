  ...
  menuGrid.innerHTML = products.map((p, i) => `
    <div class="p-card fade-in" style="transition-delay:${i * 0.07}s" id="card-${p.id}">
      ...
      <div class="p-card-body">
        ...
        <div class="p-card-actions" id="actions-${p.id}">
...
// ============================================
// REFRESH CARD ACTIONS (Agregar btn or stepper)
// ============================================
  ...
  if (qty > 0) {
    actions.innerHTML = `
      <div class="inline-stepper">
        <button onclick="cardStep(${id},-1)">&minus;</button>
        <span class="step-qty" id="sq-${id}">${qty}</span>
        <button onclick="cardStep(${id},1)">&plus;</button>
      </div>
    `;
  } else {
    actions.innerHTML = `
      <button class="add-btn" id="ab-${id}" onclick="addToCart(${id})">
        <span class="icon">${IC.plus}</span>
        Agregar
      </button>
    `;
...
// ============================================
// CARD STEPPER (+/- on the product card)
// ============================================
function cardStep(id, delta) {
  ...
  if (sqEl && item.qty > 0) {
    sqEl.textContent = item.qty;
    sqEl.classList.add('pop');
    setTimeout(() => sqEl.classList.remove('pop'), 180);
  }
